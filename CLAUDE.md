...
- Whether to go past `plaid-sync` v1 (sandbox-only, no auth, no encryption) to v2 (real Plaid Link, encrypted token storage, webhook). Gated on `auth` + hosting per `docs/security-plan.md`.
